...
# - prompt the user to select 36, 42 or 60 month repayment for the capital along with the interest
...
def loan_calculation(amount_borrowed, loan_duration)
  monthly_payment = amount_borrowed.to_f * ((0.05/12) / (1 - (1 + (0.05/12))**(-loan_duration.to_i)))
  monthly_payment
end
...
result = case loan_term
when '1'
  prompt("You have chosen 36 months loan term")
  loan_duration = 36
  puts "Your monthly payment for 36 months is $#{loan_calculation(loan_amount,loan_duration)}"
when '2'
  prompt("You have chosen 42 months loan term")
  loan_duration = 42
  puts "Your monthly payment for 42 months is $#{loan_calculation(loan_amount, loan_duration)}"
when '3'
  prompt("You have chosen 60 months loan term")
  loan_duration = 60
  puts "Your monthly payment for 60 months is $#{loan_calculation(loan_amount, loan_duration)}"
else
